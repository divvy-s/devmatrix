...
export const mediaWorker = new Worker('media:process', async (job: Job) => {
  const { mediaId } = job.data;
  const mediaArr = await db.select().from(mediaAttachments).where(eq(mediaAttachments.id, mediaId)).limit(1);
  ...
  try {
    ...
    const thumbBuffer = await image.resize(400, 400, { fit: 'cover' }).webp().toBuffer();
    ...
    await db.update(mediaAttachments)
      .set({ status: 'ready', width: metadata.width || null, height: metadata.height || null })
      .where(eq(mediaAttachments.id, mediaId));

    logger.info({ mediaId }, 'Image processed successfully');
  } catch (err: any) {
    logger.error(err, 'Media processing failed');
    await db.update(mediaAttachments).set({ status: 'failed' }).where(eq(mediaAttachments.id, mediaId));
    throw err;
  }
