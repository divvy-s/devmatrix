...
test('Webhook signatures must be computed securely via HMAC-SHA256 exactly identically to standard Web3 payload receipts', () => {
    ...
    const payload = { action: 'user.followed', data: { follower: 'A', following: 'B' } };
    ...
    const compute = crypto.createHmac('sha256', knownSecret).update(JSON.stringify(payload)).digest('hex');
    ...
    const tamperedPayload = generateSignature({ action: 'user.followed', data: { follower: 'F', following: 'B' } }, knownSecret);
