...
export class FeedService {
  async getFollowingFeed(userId: string, cursor?: string) {
    ...
    let followingStr = await redisConnection.get(cacheKey);
    let followingIds: string[] = [];

    if (followingStr) {
      followingIds = JSON.parse(followingStr);
    } else {
      const list = await db.select({ followingId: follows.followingId })
        .from(follows)
    ...
    if (dataRows.length > 0) {
      ...
      const [liked, bookmarked, reposted] = await Promise.all([
        db.select({ postId: postLikes.postId }).from(postLikes).where(and(eq(postLikes.userId, userId), inArray(postLikes.postId, postIds))),
        db.select({ postId: postBookmarks.postId }).from(postBookmarks).where(and(eq(postBookmarks.userId, userId), inArray(postBookmarks.postId, postIds))),
        db.select({ postId: postReposts.postId }).from(postReposts).where(and(eq(postReposts.userId, userId), inArray(postReposts.postId, postIds)))
      ]);
