...
export class SocialService {
  async fetchUserByUsername(username: string) {
    const arr = await db.select().from(users).where(sql`LOWER(${users.username}) = LOWER(${username})`).limit(1);
    const u = arr[0];
  ...
  async follow(userId: string, targetUsername: string) {
    ...
    if (target.id === userId) {
      throw new BusinessError('CANNOT_FOLLOW_SELF', 'You cannot follow yourself.');
    }

    // Check block
    const blockArr = await db.select().from(blocks).where(and(eq(blocks.blockerId, target.id), eq(blocks.blockedId, userId))).limit(1);
    if (blockArr.length > 0) {
      throw new BusinessError('BLOCKED', 'You have been blocked by this user.');
    }

    return await db.transaction(async (tx) => {
      const existing = await tx.select().from(follows).where(and(eq(follows.followerId, userId), eq(follows.followingId, target.id))).limit(1);

      if (existing.length > 0) {
        if (!existing[0]?.deletedAt) return { following: true }; // Idempotent
        await tx.update(follows).set({ deletedAt: null }).where(and(eq(follows.followerId, userId), eq(follows.followingId, target.id)));
      } else {
  ...
  async unfollow(userId: string, targetUsername: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    await db.update(follows).set({ deletedAt: new Date() }).where(and(eq(follows.followerId, userId), eq(follows.followingId, target.id)));
    return { following: false };
  }

  async getFollowers(targetUsername: string, requesterUserId?: string, cursorRaw?: string) {
    const target = await this.fetchUserByUsername(targetUsername);
  ...
  async getFollowing(targetUsername: string, requesterUserId?: string, cursorRaw?: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    const limitNum = 50;
  ...
  async block(userId: string, targetUsername: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    if (target.id === userId) throw new BusinessError('CANNOT_BLOCK_SELF', 'Cannot block yourself.');

    await db.transaction(async (tx) => {
      // Upsert block
      const existing = await tx.select().from(blocks).where(and(eq(blocks.blockerId, userId), eq(blocks.blockedId, target.id))).limit(1);
      if (!existing[0]) {
        await tx.insert(blocks).values({ blockerId: userId, blockedId: target.id });
      }

      // Soft delete follows
      await tx.update(follows).set({ deletedAt: new Date() }).where(
        or(
          and(eq(follows.followerId, userId), eq(follows.followingId, target.id)),
          and(eq(follows.followerId, target.id), eq(follows.followingId, userId))
        )
      );
  ...
  async unblock(userId: string, targetUsername: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    await db.delete(blocks).where(and(eq(blocks.blockerId, userId), eq(blocks.blockedId, target.id)));
  }

  async mute(userId: string, targetUsername: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    if (target.id === userId) throw new BusinessError('CANNOT_MUTE_SELF', 'Cannot mute yourself.');

    const existing = await db.select().from(mutes).where(and(eq(mutes.muterId, userId), eq(mutes.mutedId, target.id))).limit(1);
    if (!existing[0]) {
  ...
  async unmute(userId: string, targetUsername: string) {
    const target = await this.fetchUserByUsername(targetUsername);
    await db.delete(mutes).where(and(eq(mutes.muterId, userId), eq(mutes.mutedId, target.id)));
  }
