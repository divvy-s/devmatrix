...
export class AnalyticsController {
  ingestEvent = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const allowed = ['post.viewed','app.opened','app.closed','feed.scrolled','profile.viewed','search.performed','notification.clicked'];
    ...
    analyticsIngestQueue.add('ingest', payload).catch(err => {
       console.error('Failed pushing analytic event to queue asynchronously', err);
    });
