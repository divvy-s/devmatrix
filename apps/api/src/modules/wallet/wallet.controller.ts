...
export class WalletController {
  ...
  addWallet = async (request: FastifyRequest<{ Body: { address: string; signature: string; nonce: string; chainId: number } }>, reply: FastifyReply) => {
    const userId = request.user!.userId;
    const result = await this.walletService.addWallet(userId, request.body);
    return reply.send(result);
  };

  deleteWallet = async (request: FastifyRequest<{ Params: { address: string } }>, reply: FastifyReply) => {
    const userId = request.user!.userId;
  ...
  setPrimaryWallet = async (request: FastifyRequest<{ Params: { address: string } }>, reply: FastifyReply) => {
    const userId = request.user!.userId;
    const { address } = request.params;
