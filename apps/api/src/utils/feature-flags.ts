...
export async function isFeatureEnabled(name: string, user?: { userId: string, roles: string[] }): Promise<boolean> {
   ...
   if (cached) {
      ...
      const fArr = await db.select().from(featureFlags).where(eq(featureFlags.name, name)).limit(1);
      if (fArr[0]) {
         flag = fArr[0];
