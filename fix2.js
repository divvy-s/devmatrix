...
tsc = tsc.replace(/"exactOptionalPropertyTypes":\s*true/, '"exactOptionalPropertyTypes": false');
...
app = app.replace('reply.status(error.statusCode', 'reply.status(Number(error.statusCode)');
fs.writeFileSync('apps/api/src/app.ts', app);

let auth = fs.readFileSync('apps/api/src/modules/auth/auth.service.ts', 'utf8');
auth = auth.replace('expiresIn: JWT_EXPIRES_IN', 'expiresIn: JWT_EXPIRES_IN as any');
auth = auth.replace('expiresIn: JWT_EXPIRES_IN }', 'expiresIn: JWT_EXPIRES_IN as any }');
