...
for(const f of routes) {
  ...
  r = r.replace(/controller\.([a-zA-Z0-9_]+)(\s*[,)\]\n])/g, 'controller.$1 as any$2');
  fs.writeFileSync(f, r);
}

let ss = fs.readFileSync('apps/api/src/modules/social/social.service.ts', 'utf8');
ss = ss.replace('!existing[0].deletedAt', '!existing[0]?.deletedAt');
fs.writeFileSync('apps/api/src/modules/social/social.service.ts', ss);

let ws = fs.readFileSync('apps/api/src/modules/wallet/wallet.service.ts', 'utf8');
...
if (!server.includes("import { outboxWorker }")) {
  server = "import { outboxWorker } from './infrastructure/outbox.worker';\n" + server;
}
server = server.replace(/import\('\.\/infrastructure\/outbox\.worker'\)\.then\(\(\{ outboxWorker \}\) => \{[\s\n]*outboxWorker\.start\(\);[\s\n]*\}\);/g, 'outboxWorker.start();');
