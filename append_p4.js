...
let exportIdx = fs.readFileSync(path.join(__dirname, 'packages/db/src/index.ts'), 'utf8');
const exportsArr = [
  'reports', 'moderationActions', 'moderationAppeals', 'analyticsEvents',
  'trendingScores', 'appAnalyticsRollups', 'featureFlags'
];
// Append to index.ts safely
if (!exportIdx.includes('reports')) {
   fs.writeFileSync(path.join(__dirname, 'packages/db/src/index.ts'), exportIdx + '\\nexport * from \\'./schema\\';\\n');
   // Wait, export * from './schema' exports all named exports perfectly natively! I already have export * from './schema' in index.ts probably!
}
console.log('Appended safely to Drizzle');
