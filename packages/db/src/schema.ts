...
export const externalIdentities = pgTable(
  ...
  (table) => {
    return {
      ...
      providerIdIdx: uniqueIndex('ext_identities_provider_idx').on(
        table.provider,
        table.providerId,
      ),
    };
...
export const auditLogs = pgTable(
  ...
  (table) => {
    return {
      actorCreatedIdx: index('audit_actor_created_idx').on(
        table.actorId,
        table.createdAt,
      ),
...
export const follows = pgTable(
  ...
  (table) => {
    return {
      followerPk: uniqueIndex('follows_pk').on(
        table.followerId,
        table.followingId,
      ),
      followerIdx: index('follows_follower_idx').on(
        table.followerId,
        table.deletedAt,
      ),
      followingIdx: index('follows_following_idx').on(
        table.followingId,
        table.deletedAt,
      ),
    };
...
export const posts = pgTable(
  ...
  (table) => {
    return {
      contentLenCheck: check(
        'posts_content_max',
        sql`char_length(${table.content}) <= 500`,
      ),
      authorCreatedIdx: index('posts_author_created_idx')
        .on(table.authorId, table.createdAt)
        .where(sql`${table.deletedAt} IS NULL`),
      parentIdx: index('posts_parent_idx')
        .on(table.parentId)
        .where(sql`${table.parentId} IS NOT NULL`),
      rootIdx: index('posts_root_idx')
        .on(table.rootId)
        .where(sql`${table.rootId} IS NOT NULL`),
      modStatusIdx: index('posts_mod_status_idx')
        .on(table.moderationStatus)
...
export const postLikes = pgTable(
  ...
  (table) => {
    return {
      userPostUnique: uniqueIndex('post_likes_user_post_unique').on(
        table.userId,
        table.postId,
      ),
...
export const postReposts = pgTable(
  ...
  (table) => {
    return {
      userPostUnique: uniqueIndex('post_reposts_user_post_unique').on(
        table.userId,
        table.postId,
      ),
    };
...
export const notifications = pgTable(
  ...
  (table) => {
    return {
      recipientUnreadIdx: index('notifs_recipient_unread_idx').on(
        table.recipientId,
        table.readAt,
        table.createdAt,
      ),
      recipientCreatedIdx: index('notifs_recipient_created_idx').on(
        table.recipientId,
        table.createdAt,
      ),
    };
...
export const apps = pgTable(
  ...
  (table) => {
    return {
      ...
      catStatusIdx: index('apps_cat_status_idx').on(
        table.category,
        table.status,
      ),
    };
...
export const appVersions = pgTable(
  ...
  (table) => {
    return {
      appVersionUnique: uniqueIndex('app_versions_unique').on(
        table.appId,
        table.version,
      ),
    };
...
export const appPermissionGrants = pgTable(
  'app_permission_grants',
  {
    ...
    scopeId: uuid('scope_id')
      .notNull()
      .references(() => permissionScopes.id),
    grantedAt: timestamp('granted_at', { withTimezone: true })
      .defaultNow()
      .notNull(),
    revokedAt: timestamp('revoked_at', { withTimezone: true }),
  ...
  (table) => {
    return {
      userAppIdx: index('app_grants_user_app_idx').on(
        table.userId,
        table.appId,
      ),
      userAppScopeIdx: index('app_grants_user_app_scope_idx').on(
        table.userId,
        table.appId,
        table.scopeId,
      ),
...
export const appInstalls = pgTable(
  'app_installs',
  {
    userId: uuid('user_id')
      .notNull()
  ...
  (table) => {
    return {
      ...
      userUninstIdx: index('app_installs_user_uninst_idx').on(
        table.userId,
        table.uninstalledAt,
      ),
      appUninstIdx: index('app_installs_app_uninst_idx').on(
        table.appId,
        table.uninstalledAt,
      ),
    };
...
export const appStorage = pgTable(
  ...
  (table) => {
    return {
      appUserKeyUnique: uniqueIndex('app_storage_app_user_key_unique').on(
        table.appId,
        table.userId,
        table.key,
      ),
      appUserIdx: index('app_storage_app_user_idx').on(
        table.appId,
        table.userId,
      ),
    };
...
export const appTokens = pgTable(
  ...
  (table) => {
    return {
      appUserIdx: index('app_tokens_app_user_idx').on(
        table.appId,
        table.userId,
      ),
    };
...
export const webhookSubscriptions = pgTable(
  ...
  (table) => {
    return {
      appEventUnique: uniqueIndex('webhook_sub_app_evt_unique').on(
        table.appId,
        table.eventType,
      ),
    };
...
export const reports = pgTable(
  ...
  (table) => {
    return {
      statusIdx: index('reports_status_time_idx').on(
        table.status,
        table.createdAt,
      ),
      targetIdx: index('reports_target_idx').on(
        table.targetType,
        table.targetId,
      ),
    };
...
export const moderationActions = pgTable(
  ...
  (table) => {
    return {
      targetIdx: index('mod_actions_target_time_idx').on(
        table.targetType,
        table.targetId,
        table.createdAt,
      ),
      adminIdx: index('mod_actions_admin_time_idx').on(
        table.adminId,
        table.createdAt,
      ),
    };
...
export const moderationAppeals = pgTable(
  ...
  (table) => {
    return {
      userActionUnique: uniqueIndex('mod_appeals_user_action_unique').on(
        table.userId,
        table.actionId,
      ),
    };
...
export const analyticsEvents = pgTable(
  ...
  (table) => {
    return {
      eventCreatedIdx: index('analytics_event_created_idx').on(
        table.eventType,
        table.createdAt,
      ),
      userCreatedIdx: index('analytics_user_created_idx').on(
        table.userId,
        table.createdAt,
      ),
...
export const trendingScores = pgTable(
  'trending_scores',
  {
    resourceType: text('resource_type').notNull(),
    resourceId: text('resource_id').notNull(),
...
export const appAnalyticsRollups = pgTable(
  'app_analytics_rollups',
  {
    id: uuid('id')
      .primaryKey()
      .default(sql`gen_random_uuid()`),
  ...
  (table) => {
    return {
      appPeriodUnique: uniqueIndex('app_analytics_unique').on(
        table.appId,
        table.period,
        table.periodStart,
      ),
