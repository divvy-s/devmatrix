    ...
    return await db.transaction(async (tx) => {
      ...
      if (dev.userId === adminUserId) {
        throw new BusinessError(
          'Developers cannot approve their own apps',
      ...
      const ver = verArr[0];
      if (!ver)
        throw new BusinessError('No version to approve', 'MISSING_VERSION');

      await tx
        .update(appVersions)
        .set({ status: 'approved', approvedAt: new Date() })
        .where(eq(appVersions.id, ver.id));
  ...
  async suspendApp(appId: string, reason: string) {
    return await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(eq(appTokens.appId, appId));
