...
export class WalletService {
  ...
  async addWallet(
    ...
    return await db.transaction(async (tx) => {
      ...
      const existingOwnerArr = await tx
        .select({ userId: walletAddresses.userId })
        .from(walletAddresses)
  ...
  async deleteWallet(userId: string, address: string) {
    ...
    await db.transaction(async (tx) => {
      const walletArr = await tx
        .select()
        .from(walletAddresses)
        .where(
          and(
            eq(walletAddresses.userId, userId),
            eq(walletAddresses.address, normalized),
          ),
        )
        .limit(1);
      if (!walletArr[0]) {
      ...
      const result = await tx
        .select({ value: count(externalIdentities.id) })
        .from(externalIdentities)
        .where(eq(externalIdentities.userId, userId));
      const totalIdentities = result[0]?.value;
      if ((totalIdentities || 0) <= 1) {
        throw new BusinessError(
          'LAST_IDENTITY_CANNOT_REMOVE',
          'Cannot remove the last identity of an account',
        );
      }
      ...
      await tx
        ...
        .where(
          and(
            ...
            eq(externalIdentities.provider, 'wallet'),
            eq(externalIdentities.providerId, normalized),
          ),
        );
    });
  }
