...
export class ConsentController {
  ...
  getConsentDetails = async (
    ...
    const result = await this.service.getConsentDetails(
      request.user!.userId,
      request.params.slug,
    );
  ...
  installApp = async (
    request: FastifyRequest<{ Params: { slug: string }; Body: any }>,
    reply: FastifyReply,
  ) => {
    const scopes = (request.body as any).grantScopes || [];
    const result = await this.service.installApp(
      request.user!.userId,
      request.params.slug,
      scopes,
    );
  ...
  revokePermissions = async (
    ...
    await this.service.revokePermissions(
      request.user!.userId,
      request.params.slug,
      scopes,
    );
