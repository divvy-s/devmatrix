...
export class DevelopersService {
  async register(
    userId: string,
    data: { displayName: string; websiteUrl?: string; bio?: string },
  ) {
    return await db.transaction(async (tx) => {
      const devCheck = await tx
        .select()
        .from(developers)
  ...
  async getAppAnalytics(
    ...
    const appCheck = await db
      .select({ id: apps.id })
      .from(apps)
      .where(and(eq(apps.id, appId), eq(apps.developerId, dev.id)))
      .limit(1);
    ...
    return {
      ...
      webhookSuccessRate: rows.map(
        (r: any) =>
          r.webhookDeliveries / (r.webhookDeliveries + r.webhookFailures || 1),
      ),
