...
export class ModerationService {
  ...
  async resolveReport(
    reportId: string,
    adminId: string,
    data: { action: string; note: string; targetAction?: any },
  ) {
    return await db.transaction(async (tx) => {
      const rep = await tx
        .select()
      ...
      if (data.targetAction) {
        await tx.insert(moderationActions).values({
          ...
          note: data.note,
          expiresAt: data.targetAction.expiresAt
            ? new Date(data.targetAction.expiresAt)
            : undefined,
          reportId,
        });
      }
  ...
  async shadowbanUser(userId: string, adminId: string) {
    return await db.transaction(async (tx) => {
      const u = await tx
        .update(users)
        .set({ moderationStatus: 'shadowbanned' })
      ...
      await tx.insert(moderationActions).values({
        ...
        targetId: userId,
        action: 'shadowban',
        reason: 'Admin enforced shadowban',
      });
      return u[0];
    });
  }
  ...
  async getMyActions(userId: string) {
    return await db
      ...
      .where(
        and(
          eq(moderationActions.targetId, userId),
  ...
  async createAppeal(userId: string, actionId: string, reason: string) {
    const act = await db
      ...
      .limit(1);
    if (!act[0] || act[0].targetId !== userId)
      throw new BusinessError('Not your action', 'UNAUTHORIZED_APPEAL');

    return await db.transaction(async (tx) => {
      const appArr = await tx
        .insert(moderationAppeals)
        .values({ userId, actionId, reason })
        .returning();
      await tx.insert(outboxEvents).values({
        type: 'appeal.created',
        payload: { appealId: appArr[0]!.id },
      });
      return appArr[0];
    });
  }
    ...
    return await db.transaction(async (tx) => {
      ...
      if (status === 'approved') {
        const act = await tx
          .select()
          .from(moderationActions)
          .where(eq(moderationActions.id, appArr[0].actionId))
          .limit(1);
        const action = act[0];
        if (action && action.action === 'suspend') {
          ...
          await tx
            .update(posts)
            .set({ moderationStatus: 'visible' })
            .where(eq(posts.id, action.targetId));
        }
      }
