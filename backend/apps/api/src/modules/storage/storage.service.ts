...
export class StorageService {
  async getStorage(appId: string, userId: string, key: string) {
    const arr = await db
      ...
      .where(
        and(
          eq(appStorage.appId, appId),
          eq(appStorage.userId, userId),
  ...
  async putStorage(appId: string, userId: string, key: string, value: any) {
    if (key.length > 255)
      throw new BusinessError('Key exceeds 255 chars', 'INVALID_KEY');
    if (JSON.stringify(value).length > 65536)
      throw new BusinessError('Value exceeds 64KB', 'PAYLOAD_TOO_LARGE');
  ...
  async listStorage(appId: string, userId: string) {
    const arr = await db
      ...
      .from(appStorage)
      .where(and(eq(appStorage.appId, appId), eq(appStorage.userId, userId)))
      .limit(1000);
